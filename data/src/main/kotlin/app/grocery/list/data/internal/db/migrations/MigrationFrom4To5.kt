...
internal object MigrationFrom4To5 : Migration(4, 5) {
    ...
    override fun migrate(db: SupportSQLiteDatabase) {
        with(db) {
            ...
            execSQL("CREATE TABLE `custom_product_list` (`custom_product_list_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `title` TEXT NOT NULL, `color_scheme` INTEGER NOT NULL DEFAULT 0)")
            ...
            for (product in products) {
                val quotedEmojiOrNull = product.emojiAndKeyword.emoji?.let { "'$it'" } ?: "NULL"
                val quotedKeywordOrNull = product.emojiAndKeyword.keyword?.let { "'$it'" } ?: "NULL"
                execSQL(
                    """
                        INSERT INTO `product`(`product_id`, `title`, `emoji`, `keyword`, `enabled`, `non_fk_category_id`)
                             VALUES (${product.id}, '${product.title}', $quotedEmojiOrNull, $quotedKeywordOrNull, ${if (product.enabled) 1 else 0}, ${product.nonFkCategoryId})
                    """.trimIndent()
                )
            }
        }
